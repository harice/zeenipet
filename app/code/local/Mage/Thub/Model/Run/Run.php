...
//  protected $status_list = array("NEW","COMPLETE","CLOSED","PENDING", "PROCESSING", "HOLDED", "CANCELED");
 ...
 protected   $PayMethods = array(      'paypal_express'      => 'PayPal Express',
	...
	protected $payment_credit_card_info = array();
	/**
...
//$thesearch =array(array('updated_at'=>array('from'=>"$thedate")));
      ...
      if((int)$this->QB_ORDER_START_NUMBER == 0){
       	if (isset($this->RequestParams['NUMBEROFDAYS'])){
			...
			if (isset($this->RequestParams['DOWNLOADSTARTDATE'])){
				...
				$thedate = date("Y-m-d H:i:s", strtotime($date1));
      ...
      public  function  CreateHeaderXml(){
         ...
         $this->xmlResponse->encoding='ISO-8859-1';
      	...
      	$this->root = $this->xmlResponse->createTag("RESPONSE", array('Version'=>'4.6'));
...
//MJ//                  
$sqlOrders =  $this->_orders->getSelect();
//echo $sqlOrders;
//MJ
  //        exit;
 ...
 // --------- MB 18/02/2009 start --------
            ...
            $dateTimeCreateOrder_=date("Y-m-d H:i:s", $timestamp_convert_data);
...
// --------- MB 18/02/2009 end ----------
 ...
 //         $this->xmlResponse->createTag("SalesRep",         array(), "not found",            $orderNode,  $this->Get__ENCODE_RESPONSE());
           ...
           // Start - To Get Gift message for hole order <Ganesh> 08/11/2010
		    ...
		    if (isset($orders['onestepcheckout_customercomment'])&&$orders['onestepcheckout_customercomment']!='')
		    {
               $comment1 .= $orders['onestepcheckout_customercomment']."\r\n";
		    }
		   ...
		   // Start - To Create Custom fields and assign customer id in customfield1 tag <Ganesh> 09/08/2011
			$CustomValue = "";
			for ($i=1; $i<6; $i++) {
			
			if($i == 1)
			$CustomValue = $orders['customer_id'];
			else
			$CustomValue = "";
			
			$this->xmlResponse->createTag("CustomField".(string)$i,      array(), $CustomValue,  $orderNode);
            
			}
			// End - To Create Custom field and assign customer id in customfield1 tag <Ganesh> 09/08/2011
    		...
    		/*
			//Start - Ganesh 21Feb2014
			//$this->xmlResponse->createTag("PayMethod", array(),$this->getPayMethodName($payment['method']),$BillNode, $this->Get__ENCODE_RESPONSE());
			if (isset($payment['cc_type']) && $payment['cc_type'] != '' && $payment['cc_type']!=null)
			{
                if ($cc_type=$this->getCcTypeName($payment['cc_type']))
			    {
			      $this->xmlResponse->createTag("PayMethod", array(),$cc_type,$BillNode, $this->Get__ENCODE_RESPONSE());
				}
            }
			else
			{
            ...
            //End - Ganesh 21Feb2014
            */
 			
			if (isset($payment['po_number'])){
            	$this->xmlResponse->createTag("PONumber", array(),$payment['po_number'],$BillNode);
          	}
            ...
            if (isset($orders["customer_email"])){
               $this->xmlResponse->createTag("Email", array(), $this->_maxlen($orders["customer_email"],150), $BillNode, $this->Get__ENCODE_RESPONSE());
				...
				//Start - Ganesh 28Mar2012 - Code added to get additional information for credit card  
				if(isset($payment['additional_information']))
				$_payment_addtional_info = $payment['additional_information'];
				
				//Variable defined to assign payment additional information
				$Payment_cc_type = "";
				$Payment_cap_amt = "";
				...
				if(isset($_payment_addtional_info['authorize_cards']))
				{
				    $payment_authorize_cards = $_payment_addtional_info['authorize_cards'];
					foreach($payment_authorize_cards as $list => $info)
					{
						$payment_credit_card_info = $info;
						
						//Start - Ganesh 07May2012
						if(isset($payment_credit_card_info['cc_type']))
						$Payment_cc_type = $payment_credit_card_info['cc_type'];
												
						if(isset($payment_credit_card_info['captured_amount']))
						$Payment_cap_amt = $payment_credit_card_info['captured_amount'];
						else if (isset($payment_credit_card_info['processed_amount']))
						$Payment_cap_amt = $payment_credit_card_info['processed_amount'];
						
						if(isset($payment_credit_card_info['cc_exp_month']))
						$Payment_cc_exp_month = $payment_credit_card_info['cc_exp_month'];
						
						if(isset($payment_credit_card_info['cc_exp_year']))
						$Payment_cc_exp_year = $payment_credit_card_info['cc_exp_year'];
						
						if(isset($payment_credit_card_info['cc_owner']))
						$Payment_cc_owner = $payment_credit_card_info['cc_owner'];
						
						if(isset($payment_credit_card_info['cc_last4']))
						$Payment_cc_last4 = $payment_credit_card_info['cc_last4'];
						
						if(isset($payment_credit_card_info['last_trans_id']))
						$Payment_last_trans_id = $payment_credit_card_info['last_trans_id'];
						//End - Ganesh 07May2012
					}
				}
				//End - Ganesh 28Mar2012 - Code added to get additional information for credit card
									   
				//Start - Ganesh 21Feb2014
				//$this->xmlResponse->createTag("PayMethod", array(),$this->getPayMethodName($payment['method']),$BillNode, $this->Get__ENCODE_RESPONSE());
				if($Payment_cc_type !=null && $Payment_cc_type !="")
				{
					if ($cc_type=$this->getCcTypeName($Payment_cc_type))
					{
					  $this->xmlResponse->createTag("PayMethod", array(),$cc_type,$BillNode, $this->Get__ENCODE_RESPONSE());
					}
				}
               ...
               if (isset($payment['amount_paid'])){
                 ...
                 //Start - Ganesh 28Mar2012 - Code added to get additional information for credit card      
				if($Payment_cap_amt!=null && $Payment_cap_amt!="" ){
			    $this->xmlResponse->createTag("CreditCardCharge", array(), $Payment_cap_amt, $BillCreditCardNode, $this-> Get__ENCODE_RESPONSE());
				}else{
                ...
                if (isset($payment['cc_exp_month'])&&isset($payment['cc_exp_year'])){
                  $this->xmlResponse->createTag("ExpirationDate", array(),sprintf('%02d',$payment['cc_exp_month']).substr($payment['cc_exp_year'],-2,2), $BillCreditCardNode, $this->Get__ENCODE_RESPONSE());
                    ...
                    //Start - Ganesh 28Mar2012 - Code added to get additional information for credit card
					if ($Payment_cc_exp_month !="" && $Payment_cc_exp_year!="")
					{
                    $this->xmlResponse->createTag("ExpirationDate", array(),sprintf('%02d',$Payment_cc_exp_month).substr($Payment_cc_exp_year,-2,2), $BillCreditCardNode, $this->Get__ENCODE_RESPONSE());
					}else{
				...
				{
				$CreditCardNumber = ($Payment_cc_last4 !=""?$this->getCcNumberXXXX($Payment_cc_last4):"");
				}
				//End - Ganesh 28Mar2012 - Code added to get additional information for credit card
				...
				{
				$AuthDetails .= ($Payment_last_trans_id!="" ?"last_trans_id=".$Payment_last_trans_id.";":"");
				}
				//End - Ganesh 28Mar2012 - Code added to get additional information for credit card
				...
				*********************************************************************************************************/                  
					...
					if($ItemProductType == 'configurable')
					{
					  ...
					  if(isset($itm['has_children'])&&$itm['has_children']==true){
						 if(isset($itm['product_type'])&&  ($itm['product_type']=='configurable' || $itm['product_type']=='bundle')   ){
						...
						//Start - Ganesh 09/24/2013
                        //Note - 1.One line for bundle products like it was before, 2.Display parent with 0 price and quantity and then sub items, 3.Display only sub products
						//    $_BUNDLEPRODUCT_FLAG = '1';
						  $_BUNDLEPRODUCT_FLAG = '2';
						//  $_BUNDLEPRODUCT_FLAG = '3';
						//End - Ganesh 09/24/2013
						...
						// ***  UNCOMMENT THE FOLLOWING SECTION IF BUNDLED PRODUCT SHOULD BE POSTED TO qb AS ZERO QUANTITY AND AMOUNT
						 ...
						 {
						    //Start - Ganesh 09/24/2013
						    if( $_BUNDLEPRODUCT_FLAG != '1')  
							{
							    $BundleSubItemOrder_Qty = $itm["qty_ordered"]; //Ganesh - 21Feb2014
								
								 //Ganesh - 20Mar2014
						    	$BundleItemBaseTotal=0;
								if(isset($itm["base_row_total"]) && $itm["base_row_total"] !='')
								{
								  $BundleItemBaseTotal = $itm["base_row_total"]; 
								}
								//End - 20Mar2014
			        ...
			        if(isset($itm['product_type']) && $itm['product_type'] != 'bundle')
					{
				        ...
				        $ItemDescription = $this->_maxlen($itm["name"],250).(isset($itm["description"])?(" ".$itm["description"]):"");
						...
						// Start - To Assign Item cost <Ganesh> 08/07/2011     
						if (isset($itm["base_cost"]) && $itm["base_cost"]!=null){
						$this->xmlResponse->createTag("UnitCost",      array(),  $itm["base_cost"],     $itemNode);
						}								   
						// End - To Assign Item cost <Ganesh> 08/07/2011   
			        ...
			        if(isset($itm['product_type']) && $itm['product_type'] == 'bundle' && $_BUNDLEPRODUCT_FLAG != '3')
			        {
					 	$itemNode = $this->xmlResponse->createTag("Item",    array(), '',    $itemsNode);
						$this->xmlResponse->createTag("ItemType",       array(), $ItemProductType, $itemNode);
						// $this->xmlResponse->createTag("cofigProdUnitPrice",       array(), $cofigProdUnitPrice, $itemNode);
					...
					if(isset($itm['product_type']) && $itm['product_type'] == 'bundle' && $_BUNDLEPRODUCT_FLAG != '1')   
					{
						if(isset($itm['product_options']) && $itm['product_options']!=='')
						{
							$bundle_options_list = unserialize($itm['product_options']);
											
							if(isset($bundle_options_list['bundle_options'])&& $bundle_options_list['bundle_options']!=='')
							{
								$bundle_options = $bundle_options_list['bundle_options'];
												
								if (count($bundle_options)>0)
								{
									foreach($bundle_options as $_bundle_options_Item)
									{
										$bundle_options_value = $_bundle_options_Item['value'];
															
										$itemNode = $this->xmlResponse->createTag("Item", array(), '',    $itemsNode);
										$this->xmlResponse->createTag("ItemType", array(), $itm['product_type'], $itemNode);
										$this->xmlResponse->createTag("ItemCodeParent", array(), $itm["sku"], $itemNode,$this->Get__ENCODE_RESPONSE());
												 
										$bundle_item_description= "";
										foreach($bundle_options_value as $_bundle_options_value)
										{
										 $bundle_itemCode = explode(",", $_bundle_options_value['title']);
										
										 $item_desc = "";
										 if(($pos = strpos($_bundle_options_value['title'], ',')) !== false)
										 {
											$item_desc = substr($_bundle_options_value['title'], $pos + 1);
										 }
										 
										 $bundle_item_description = $_bundle_options_Item['label'] . "-" . $item_desc;

										 //Start - <TIBB> 20Mar2014 - Code to find SKU Between []
										 $BundleCode = $bundle_itemCode[0];
										 $findme   = '[';
										 $posCode = strpos($BundleCode, $findme);
										 
										 if ($posCode === false)
										 {
											$SKUBET = $bundle_itemCode[0];
										 }
										 else
										 {
											 $temp1 = strpos($bundle_itemCode[0],"[")+strlen("[");
											 //echo "Temp :".$temp1."<br/>'";
											 
											 $result = substr($bundle_itemCode[0],$temp1,strlen($bundle_itemCode[0]));
											...
											//echo "DD2 :".$dd."<br/>'";
											$SKUBET = substr($result,0,$dd);
										}
										//End - <TIBB> 20Mar2014
										 
										 //echo "SKUBET :".$SKUBET."<br/>'";
										 //$this->xmlResponse->createTag("ItemCode", array(), $bundle_itemCode[0], $itemNode, $this->Get__ENCODE_RESPONSE());
										 
										 $this->xmlResponse->createTag("ItemCode", array(), $SKUBET, $itemNode, $this->Get__ENCODE_RESPONSE());
										 $this->xmlResponse->createTag("ItemDescription", array(), $bundle_item_description, $itemNode, $this->Get__ENCODE_RESPONSE()); 
										 										 
										 //Start - Ganesh 21Feb2014 
										 //echo "ITEM QTY: ".$BundleSubItemOrder_Qty."<br/>";
										 //echo "Bundle Qty :".$_bundle_options_value['qty']."<br/>";
										 
										 $Qty = 0;
										 if($_bundle_options_value['qty'] != '')
										 {
										   //echo "1 <br/>";
										   $Qty = $_bundle_options_value['qty'];
										 }
										 
										 if($BundleSubItemOrder_Qty != 0)
										 {
										   //echo "2 <br/>";
										   $Qty = $_bundle_options_value['qty'] * $BundleSubItemOrder_Qty;
										 }
										 
										 //echo "Total Qty : ". $Qty."<br/>";
										 $this->xmlResponse->createTag("Quantity", array(), $Qty, $itemNode);
										 //echo "PRICE :". $_bundle_options_value['price'];
										 //echo "ITEM TOTAL: ".$BundleItemBaseTotal."<br/>";
										 
										 $unitprice = 0.0;
										 //Start - <TIBB> 20Mar2014
										 if($BundleItemBaseTotal != 0 && $_bundle_options_value['qty'] != '')
										 {
											$unitprice = $BundleItemBaseTotal / $Qty;
										 }
										 //End - <TIBB> 20Mar2014
										 elseif($_bundle_options_value['price'] != '')
										 {
										   if($BundleSubItemOrder_Qty != 0)
										   {
										      $unitprice = ($_bundle_options_value['price'] * $BundleSubItemOrder_Qty) / $Qty;
										   }
										   else
										   {
										      $unitprice = $_bundle_options_value['price'] / $Qty;
										   }
										 }
										 //echo $unitprice;
										 //exit;
										 
										 $this->xmlResponse->createTag("UnitPrice", array(), $unitprice, $itemNode);
										 //End - Ganesh 21Feb2014
										}
									}
								}
...
//             $this->xmlResponse->createTag("FeeValue",       array(), '', $FeeDetailsNode);
 ...
 // Start - To create GiftCertificate Tag  <Ganesh> 09/10/2010  - Code changes for adding GCamount with Total  <Ganesh> 20/01/2012		
			...
			$GCamount = $orders["giftcert_amount"];
			...
			$GCamount = $orders["giftcert_amount_invoiced"];
			...
			$GCamount = $orders["base_giftcert_amount_invoiced"];
			$this->xmlResponse->createTag("GiftCertificate",       array(), $orders["base_giftcert_amount_invoiced"], $chargesNode);
            }				
            
			if(isset($GCamount)&&$GCamount!=null){
			$tot = $orders["base_grand_total"] + $GCamount;
			}else{
			$tot = $orders["base_grand_total"];
			}
			
			$this->xmlResponse->createTag("Total", array(),  $tot , $chargesNode);
			// End - To create GiftCertificate Tag  <Ganesh> 09/10/2010 - Code changes for adding GCamount with Total  <Ganesh> 20/01/2012		
