...
class Indies_Partialsubscription_Model_Payment_Method_ProtxDirect extends Indies_Partialsubscription_Model_Payment_Method_Abstract
{
    ...
    public function processOrder(Mage_Sales_Model_Order $PrimaryOrder, Mage_Sales_Model_Order $Order = null)
    {
        ...
        if (empty($ready)) {
            throw new Indies_Partialsubscription_Exception($this->__("Order cannot be completed. Unknown error"));
        }

        if ($ready['Status'] != 'OK') {
            throw new Indies_Partialsubscription_Exception($ready['Status'] . " - " . $ready['StatusDetail']);
        }
    }
