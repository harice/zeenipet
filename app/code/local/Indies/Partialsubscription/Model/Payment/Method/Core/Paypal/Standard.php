...
class Indies_Partialsubscription_Model_Payment_Method_Core_Paypal_Standard extends Mage_Paypal_Model_Standard
{
    /**
        ...
        if (isset($IPN['subscr_id'])) {
            if ($Subscription = Mage::getModel('sarp/subscription')->load($IPN['subscr_id'], 'real_id')) {
                ...
                switch ($IPN['txn_type']) {
                    case 'subscr_cancel':
                        // Cancel subscription
                        $Subscription->setStatus(Indies_Partialsubscription_Model_Subscription::STATUS_CANCELED)->save();
                    ...
                    case 'subscr_eot':
                        // End Of time
                        $Subscription->setStatus(Indies_Partialsubscription_Model_Subscription::STATUS_EXPIRED)->save();
                        // Set last oder status as canceled
                        if ($Order) {
                    ...
                    case 'subscr_failed':
                        // Failed
                        $Subscription->setStatus(Indies_Partialsubscription_Model_Subscription::STATUS_SUSPENDED)->save();
                        // Set last oder status as canceled
                        if ($Order) {
                    ...
                    default:
                        ...
                        if ($Order) {
                            $Order->setState(
                                ...
                                Mage::helper('partialsubscription')->__('Subscription payment successfully processed at PayPal'),
                                true
                            )->save();
                        }
    ...
    {
        if (Mage::helper('partialsubscription')->extensionEnabled('AW_Points') && Mage::helper('points')->magentoLess14()) {
