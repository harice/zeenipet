<?php
class Indies_Partialsubscription_Model_Web_Service_Nvp_Client extends Indies_Partialsubscription_Model_Web_Service_Client
{
     ...
     * Runs request to API.
     * @param string $method
     * @throws Indies_Partialsubscription_Exception
     * @return array
     */
    public function _runRequest($method)
    {
        ...
        if (Indies_Partialsubscription_Model_Payment_Method_Paypal_Direct::ENDPOINT_SANDBOX == $this->getAPIEndpoint()) {
        ...
        if (!$httpResponse) {
            throw new Indies_Partialsubscription_Exception("PayPal Direct $method failed: " . curl_error($ch) . '(' . curl_errno($ch) . ')');
        }
        ...
        if ((0 == sizeof($httpParsedResponseAr)) || !array_key_exists('ACK', $httpParsedResponseAr)) {
            throw new Indies_Partialsubscription_Exception("PayPal Direct: Invalid HTTP Response for POST to $API_Endpoint.");
        }
