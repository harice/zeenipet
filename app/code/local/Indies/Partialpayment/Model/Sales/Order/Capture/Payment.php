...
class Indies_Partialpayment_Model_Sales_Order_Capture_Payment
    extends Mage_Sales_Model_Order_Payment
{
    ...
    {
        $this->_init('partialpayment/sales_order_capture_payment');
    }
    ...
    protected function _lookupTransaction($txnId, $txnType = false)
    {
        ...
        $txn = Mage::getModel('partialpayment/sales_order_capture_payment_transaction')
    ...
    public function refund($creditmemo)
    {
        ...
        if ($gateway->canRefund()) {
            ...
            $isOnline = true;
            $captureTxn = Mage::getModel('partialpayment/sales_order_capture_payment_transaction')
    ...
    protected function _addTransaction($type, $salesDocument = null, $failsafe = false)
    {
		...
		Indies_Fee_Model_Fee::$transactionId = $transactionId;
        if (null !== $transactionId) {
            ...
            if (!$transaction) {
                $transaction = Mage::getModel('partialpayment/sales_order_capture_payment_transaction')->setTxnId($transactionId);
            }
            $transaction
                ->setOrderPaymentObject($this)
                ->setTxnType($type)
