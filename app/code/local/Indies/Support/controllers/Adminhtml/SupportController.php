...
class Indies_Support_Adminhtml_SupportController extends Mage_Adminhtml_Controller_action
{
    ...
    public function massDeleteAction() {
        ...
        if(!is_array($supportIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($supportIds as $supportId) {
                    $support = Mage::getModel('support/support')->load($supportId);
                    $support->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully deleted', count($supportIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($supportIds)) {
            ...
            try {
                foreach ($supportIds as $supportId) {
                    $support = Mage::getSingleton('support/support')
                        ->load($supportId)
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($supportIds))
                );
            } catch (Exception $e) {
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('support/adminhtml_support_grid')
            ->getCsv();
    ...
    {
        //return Mage::getSingleton('admin/session')->isAllowed('catalog/customoptionstemplate');
		return true;
    }
}