 ...
 * File path:   js/aitoc/aitcg/raphael.js
...
(function () {
    ...
    if (R.svg) {
        ...
        Element[proto].getBBox = function () {
            ...
            try {
                ...
                // start aitoc fix for ie9 and CHROME start
                
                if ((Browser.Version() == 9 || navigator.userAgent.match('Chrome') || /Konqueror|Safari|KHTML/.test( navigator.userAgent ))&&(this.type != "text")){
                    bbox = {x: this.attrs.x, y: this.attrs.y, width: this.attrs.width, height: this.attrs.height};
                }
                //end aitoc fix for ie9

            } catch(e) {
                // Firefox 3.0.x plays badly here
                /*a very bad hack by BlackFox START*/
                bbox = {x: this.attrs.x, y: this.attrs.y, width: this.attrs.width, height: this.attrs.height};
                /*a very bad hack by BlackFox END*/
            } finally {
                bbox = bbox || {};
            ...
            if ((Browser.Version() < 7)&&(this.type == "text")) {
        ...
        theImage = function (svg, src, x, y, w, h, aspRatio) {
            var el = $("image");
            aspRatio = (aspRatio == undefined || aspRatio == 0 || aspRatio == '')?"none":aspRatio;
            $(el, {x: x, y: y, width: w, height: h, preserveAspectRatio: aspRatio});
            el.setAttributeNS(svg.xlink, "href", src);
            svg.canvas && svg.canvas[appendChild](el);
            var res = new Element(el, svg);
            res.attrs = {x: x, y: y, width: w, height: h, src: src, preserveAspectRatio: aspRatio};
            res.type = "image";
            return res;
        },
    ...
    if (R.vml) {
        ...
        theImage = function (vml, src, x, y, w, h, aspRatio) {
            aspRatio = (aspRatio == undefined || aspRatio == 0 || aspRatio == '')?"none":aspRatio;
            var g = createNode("group"),
                o = createNode("image");
            g.style.cssText = "position:absolute;left:0;top:0;width:" + vml.width + "px;height:" + vml.height + "px";
            ...
            res.attrs.w = w;
            res.attrs.h = h;
            res.attrs.preserveAspectRatio = aspRatio;
    ...
    paperproto.image = function (src, x, y, w, h, aspRatio) {
        return theImage(this, src || "about:blank", x || 0, y || 0, w || 0, h || 0, aspRatio || 'none');
    };
