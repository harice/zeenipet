...
function formSubmit18(submitter) {
	//getting list of elements
	var nodes = document.getElementById('sfg_fieldset18').childNodes;
	var objects = Array();
	for (var i=0; i<nodes.length; i++) if (nodes[i].tagName) objects[objects.length] = nodes[i];
	// perform validation
	var isFormValid = true;
	for (var vl=0; vl<sfg_validation18.length; vl++) if (window['validator18_'+sfg_validation18[vl][1]] && objects[sfg_validation18[vl][0]]) {
		var msg = window['validator18_'+sfg_validation18[vl][1]](objects[sfg_validation18[vl][0]], sfg_validation18[vl][2], sfg_validation18[vl][3] ? sfg_validation18[vl][3] : '');
		if (msg) {
			isFormValid = false;
			alert(msg);
			objects[sfg_validation18[vl][0]].focus();
			return;
		}
	}
	document.getElementById('sfg_submitter18').value = submitter;
	document.sfgForm18.submit();
}
