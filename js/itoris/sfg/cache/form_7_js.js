...
function formSubmit7(submitter) {
	//getting list of elements
	var nodes = document.getElementById('sfg_fieldset7').childNodes;
	var objects = Array();
	for (var i=0; i<nodes.length; i++) if (nodes[i].tagName) objects[objects.length] = nodes[i];
	// perform validation
	var isFormValid = true;
	for (var vl=0; vl<sfg_validation7.length; vl++) if (window['validator7_'+sfg_validation7[vl][1]] && objects[sfg_validation7[vl][0]]) {
		var msg = window['validator7_'+sfg_validation7[vl][1]](objects[sfg_validation7[vl][0]], sfg_validation7[vl][2], sfg_validation7[vl][3] ? sfg_validation7[vl][3] : '');
		if (msg) {
			isFormValid = false;
			alert(msg);
			objects[sfg_validation7[vl][0]].focus();
			return;
		}
	}
	document.getElementById('sfg_submitter7').value = submitter;
	document.sfgForm7.submit();
}
