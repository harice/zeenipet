...
function formSubmit1(submitter) {
	//getting list of elements
	var nodes = document.getElementById('sfg_fieldset1').childNodes;
	var objects = Array();
	for (var i=0; i<nodes.length; i++) if (nodes[i].tagName) objects[objects.length] = nodes[i];
	// perform validation
	var isFormValid = true;
	for (var vl=0; vl<sfg_validation1.length; vl++) if (window['validator1_'+sfg_validation1[vl][1]] && objects[sfg_validation1[vl][0]]) {
		var msg = window['validator1_'+sfg_validation1[vl][1]](objects[sfg_validation1[vl][0]], sfg_validation1[vl][2], sfg_validation1[vl][3] ? sfg_validation1[vl][3] : '');
		if (msg) {
			isFormValid = false;
			alert(msg);
			objects[sfg_validation1[vl][0]].focus();
			return;
		}
	}
	document.getElementById('sfg_submitter1').value = submitter;
	document.sfgForm1.submit();
}
