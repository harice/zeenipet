...
class CustomImages
{
    ...
    public function createDirectory($path2){
        $flag = true;

        if(!file_exists($_SERVER['DOCUMENT_ROOT'].$path2)){
            
            if(!mkdir($_SERVER['DOCUMENT_ROOT'].$path2, 0777))
                $flag = false;
    ...
    public function displayImages($itemid){
        $html = "";
   
        if ($handle = opendir($this->path.$itemid)) {
            
            while (false !== ($file = readdir($handle))) {
                if($file!="." && $file!=".."){
                     $html .= "<img class=\"customImages deleteimage\" src=\"" . $this->http_path .$itemid."/". $file . "\" />";
                }
            }    
            closedir($handle);
        }
    ...
    public function uploadCustomImage($path2){
                ...
                if(in_array($ext,$this->valid_formats))
                {
                    if($size<(1024*1024)) // Image size max 1 MB
                    {
                        ...
                        if($this->createDirectory($path2)){
                            ...
                            $html = "Directory " .$path2. $this->path . " could not be created!";
                    }
                    else
    ...
    public function createCustomImageModule($invoiceID,$itemid){
        $html = "<fieldset>";
        $html .= "<legend>CUSTOM IMAGES</legend>";  

        $html .= "<form id=\"imageform".$itemid."\" method=\"post\" enctype=\"multipart/form-data\" action='upload/ajaximage.php?invoiceNo=$invoiceID&itemid=$itemid'>
                    <br><b>Upload custom uniform image</b> <input type=\"file\" name=\"photoimg\" id=\"photoimg".$itemid."\" /></form>";
        $html .= "<div id='preview".$itemid."'></div>";
        $html .= "<br>";
        $html .= $this->displayImages($itemid);
        $html .= "<div class=\"clear\"></div>";
        $html .= "</fieldset>";
